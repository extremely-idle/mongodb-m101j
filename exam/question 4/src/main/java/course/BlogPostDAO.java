...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
        ...
        postsCollection.find(eq("permalink", permalink)).projection(slice("comments", ordinal, (ordinal + 1))).into(posts);
        for (Document post : posts) {
            List<Document> comments = (ArrayList<Document>) post.get("comments");
            for (Document comment : comments) {
...
//                System.out.print("body => " + comment.get("likes"));
            }
        }
        postsCollection.updateOne(eq("permalink", permalink)
